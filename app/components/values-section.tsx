...
export function ValuesSection({ section }: { section: Extract<ProfileSection, { type: "values" }> }) {
  return (
    <section id={section.id} className="purpose-section" aria-labelledby={`${section.id}-title`}>
      ...
      <div className="shell purpose-stage">
        {[
          { label: "Our vision", text: section.vision, image: "/assets/vision-scene.webp", icon: Eye },
          { label: "Our mission", text: section.mission, image: "/assets/mission-scene.webp", icon: Compass },
        ].map((chapter, index) => <article className="purpose-scene" key={chapter.label}>
          <div className="purpose-backdrop" aria-hidden="true"><Image src={chapter.image} alt="" fill sizes="(max-width: 760px) 100vw, 600px" /></div>
