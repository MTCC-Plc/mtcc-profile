...
export function SustainabilitySection({ section }: { section: Extract<ProfileSection, { type: "content" }> }) {
  return <section id={section.id} className="esg-section" aria-labelledby="esg-title"><div className="shell">
    ...
    {section.blocks.filter(block => block.type === "metrics").map((block,index) => <article className="esg-investment" key={index}><div className="esg-investment-copy"><h3>{block.title}</h3><dl>{block.metrics.map(metric => <div key={metric.label}><dt>{metric.label}</dt><dd>{metric.value.startsWith("MVR ") ? <><span>MVR</span>{metric.value.slice(4)}</> : metric.value}</dd>{metric.note && <p className="esg-year">{metric.note}</p>}</div>)}</dl></div><div className="esg-landscape"><Image src="/assets/corporate-hero.webp" alt="Aerial view of the Maldivian coastline and surrounding ocean" fill sizes="(max-width: 760px) 100vw, 650px" /><span>The Maldives</span></div></article>)}
