  ...
  return <section id={section.id} className="infra-section" aria-labelledby="infra-title"><div className="shell">
    ...
    {fleet?.type === "metrics" && <div className="infra-fleet">
      <div className="infra-landscape"><Image src="/assets/dredging.webp" alt="Dredging and reclamation works in the Maldives" fill sizes="(max-width: 760px) 100vw, 1200px" /><div className="infra-image-shade" />
        <div className="infra-capacity"><h3>{fleet.title}</h3><dl>{fleet.metrics.slice(0, 1).map(metric => <div key={metric.label}><dt>{metric.label}</dt><dd>{metric.value}</dd><span>{metric.note}</span></div>)}</dl></div>
        <a href="#infra-capabilities" className="infra-explore">Explore our capabilities <ArrowDownRight size={20} aria-hidden="true" /></a>
      </div>
      <dl className="infra-vessels">{fleet.metrics.slice(1).map(metric => <div key={metric.label}><dd>{metric.value}</dd><dt>{metric.label}<span>{metric.note}</span></dt></div>)}</dl>
    </div>}
    ...
    {projects?.type === "metrics" && <div className="infra-projects"><div className="infra-block-heading"><p className="eyebrow">Delivery at scale</p><h3>{projects.title}</h3></div><dl>{projects.metrics.map((metric, i) => <div key={`${metric.label}-${i}`}><dt>{metric.label}</dt><dd>{metric.value}</dd>{metric.note && <p>{metric.note}</p>}</div>)}</dl><a href="#project-breakdown">Explore the project portfolio <ArrowDownRight size={20} aria-hidden="true" /></a></div>}
  </div></section>;
}
