...
export function TransportSection({ section }: { section: Extract<ProfileSection, { type: "content" }> }) {
  return <section id={section.id} className="transport-section" aria-labelledby="transport-title">
    <header className="transport-opening">
      ...
      <div className="shell"><div className="transport-opening-scene"><div className="transport-hero-image" aria-hidden="true"><Image src="/assets/transport.webp" alt="" fill sizes="(max-width: 760px) 100vw, 1200px" /></div><div className="transport-hero-shade" /><div className="transport-opening-caption"><span>Across the sea.<br />Along the road.</span><a href="#transport-fleet">Explore the network <span aria-hidden="true">↓</span></a></div></div></div>
    </header>
    <div className="shell transport-content">
      {section.blocks.map((block, index) => {
        ...
        if (block.type === "bars") return <article className="transport-passengers" key={index}>
          ...
          {block.note && <div className="transport-total-feature"><span>Total passengers</span><strong>{block.note.match(/[\d,]+/)?.[0]}</strong></div>}
