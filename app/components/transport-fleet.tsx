...
export function TransportFleet({ block }: { block: Extract<ContentBlock, { type: "table" }> }) {
  ...
  return <article ref={root} id="transport-fleet" className="transport-fleet">
    ...
    {block.rows.map((row,index)=>{return <div key={row[0]} className={`fleet-showcase fleet-mode-${index}`} role="tabpanel" id={`fleet-panel-${index}`} aria-labelledby={`fleet-tab-${index}`} hidden={active!==index} tabIndex={0}>
      <div className="fleet-showcase-visual"><Image src={images[index]} alt={imageDescriptions[index]} fill sizes="(max-width: 760px) 100vw, 750px" /><span className="fleet-visual-number" aria-hidden="true">0{index+1} / 04</span></div>
